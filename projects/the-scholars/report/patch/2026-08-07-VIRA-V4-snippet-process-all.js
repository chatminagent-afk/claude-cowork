...
// ── SUFFIX [UNKNOWN]: arahkan user ke Sam ──────────────────────
// Ditaruh SETELAH inject GForm & SETELAH auto-capitalize, supaya:
//  - tidak dobel dgn kalimat identik yg sudah ditempel blok GForm
//  - "tinggal ketik" tidak ikut ter-kapitalisasi
const SAM_SUFFIX  = 'Kalau ada yang masih kurang jelas, bisa ngobrol langsung sama Sam yaa. tinggal ketik "mau ngomong sama Sam".';
const SAM_HINT_RE = /mau ngomong sama sam/i;

if (isUnknown && !isTalkToSam && !SAM_HINT_RE.test(cleanOutput)) {
  cleanOutput = `${cleanOutput}\n\n${SAM_SUFFIX}`;
  console.log('🔁 UNKNOWN suffix ditambahkan (arahkan ke Sam)');
}
