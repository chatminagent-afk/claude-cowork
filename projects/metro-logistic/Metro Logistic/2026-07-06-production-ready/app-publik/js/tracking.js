...
(function () {
  ...
  // [GABUNG 2026-07-06] Tombol "Terapkan Filter" dihapus — pencarian terpadu lewat btnLacak.
  ...
  // ── Paginasi hasil (model "Halaman 1 dari 20", maks 50 resi/halaman) ──
  const RESI_PER_HALAMAN = 50;
  let halamanSaatIni = 1;
  let labelAuto30Terakhir = false; // ingat label terakhir agar navigasi halaman konsisten
   ...
   */
  /** Sembunyikan bar paginasi. */
  function sembunyikanPaginasi() {
    if (!paginasi) return;
    paginasi.classList.add('hidden');
    paginasi.classList.remove('flex');
    paginasi.innerHTML = '';
  }

  /** Pindah halaman lalu render ulang (tanpa fetch), scroll ke atas hasil. */
  function gotoHalaman(n) {
    halamanSaatIni = n;
    renderHasil(labelAuto30Terakhir);
    if (judulHasil && judulHasil.scrollIntoView) {
      judulHasil.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  }

  /** Render bar paginasi model "Halaman X dari Y" + tombol Sebelumnya/Berikutnya. */
  function renderPaginasi(totalHalaman, totalResi, mulaiIdx, jumlahDitampilkan) {
    if (!paginasi) return;
    if (totalHalaman <= 1) { sembunyikanPaginasi(); return; }

    const dari = mulaiIdx + 1;
    const sampai = mulaiIdx + jumlahDitampilkan;
    const disablePrev = halamanSaatIni <= 1;
    const disableNext = halamanSaatIni >= totalHalaman;
    const kelasTombol = 'min-h-[40px] px-4 rounded-xl text-sm font-semibold border transition-all duration-200 flex items-center gap-1.5';
    const aktif = 'bg-primary text-white border-primary hover:bg-primary-light active:scale-95 shadow-sm';
    const mati = 'bg-slate-50 dark:bg-slate-800/60 text-slate-300 dark:text-slate-600 border-slate-100 dark:border-slate-800 cursor-not-allowed';

    paginasi.innerHTML = `
      <p class="text-[11px] text-slate-400 dark:text-slate-500 font-medium order-2 sm:order-1">
        Menampilkan <b class="text-slate-600 dark:text-slate-300">${dari}–${sampai}</b> dari <b class="text-slate-600 dark:text-slate-300">${totalResi}</b> resi
      </p>
      <div class="flex items-center gap-2 order-1 sm:order-2">
        <button id="btnPrevHalaman" type="button" ${disablePrev ? 'disabled' : ''} class="${kelasTombol} ${disablePrev ? mati : aktif}">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"/></svg>
          Sebelumnya
        </button>
        <span class="text-xs font-bold text-slate-500 dark:text-slate-400 px-2 whitespace-nowrap">Halaman ${halamanSaatIni} dari ${totalHalaman}</span>
        <button id="btnNextHalaman" type="button" ${disableNext ? 'disabled' : ''} class="${kelasTombol} ${disableNext ? mati : aktif}">
          Berikutnya
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/></svg>
        </button>
      </div>`;
    paginasi.classList.remove('hidden');
    paginasi.classList.add('flex');

    const prev = document.getElementById('btnPrevHalaman');
    const next = document.getElementById('btnNextHalaman');
    if (prev && !disablePrev) prev.addEventListener('click', () => gotoHalaman(halamanSaatIni - 1));
    if (next && !disableNext) next.addEventListener('click', () => gotoHalaman(halamanSaatIni + 1));
  }

  function renderHasil(labelAuto30) {
    labelAuto30Terakhir = !!labelAuto30;
    if (!hasilTerakhir || hasilTerakhir.length === 0) {
      hasilContainer.innerHTML = '';
      judulHasil.classList.add('hidden');
      sembunyikanPaginasi();
      // [GABUNG] filterSection kini bagian dari form pencarian utama — selalu tampil.
      return;
    }
    ...
    if (hasilTersaring.length === 0) {
      ...
      // ── Paginasi: potong hasil menjadi maks RESI_PER_HALAMAN per halaman ──
      const totalHalaman = Math.max(1, Math.ceil(hasilTersaring.length / RESI_PER_HALAMAN));
      if (halamanSaatIni > totalHalaman) halamanSaatIni = totalHalaman;
      if (halamanSaatIni < 1) halamanSaatIni = 1;
      const mulai = (halamanSaatIni - 1) * RESI_PER_HALAMAN;
      const potong = hasilTersaring.slice(mulai, mulai + RESI_PER_HALAMAN);
      hasilContainer.innerHTML = potong.map(result => renderCard(result, efektif)).join('');
      renderPaginasi(totalHalaman, hasilTersaring.length, mulai, potong.length);
    }
  }
  ...
  function renderErrorJaringan(pesan) {
    ...
    if (btn) btn.addEventListener('click', lakukanPencarian);
  }
  ...
  async function lakukanLacak() {
    ...
    halamanSaatIni = 1;      // pencarian baru → mulai dari halaman 1
    judulHasil.classList.remove('hidden');
    ...
    try {
      ...
      // Kontrak utama: { results: [...] }. Fallback { data: [...] } diterima
      // demi kompatibilitas bila versi workflow n8n lama masih terpasang.
  ...
  // [GABUNG 2026-07-06] Satu tombol "Lacak 🔍" untuk pencarian terpadu
  // (no resi + tanggal + status). Deklarasi lakukanPencarian ada di bawah
  // (function declaration → hoisted, aman dirujuk di sini).
  btnLacak.addEventListener('click', lakukanPencarian);
   ...
   * - Mode Uji Dummy: muat seluruh resi dummy (termasuk update dari portal admin).
   * - Mode produksi [UPDATE 2026-07-06]: panggil WF1 TANPA parameter resi —
   *   n8n membalas seluruh resi yang diupdate dalam 30 hari terakhir
   *   (maks 100, terbaru dulu), sehingga perilaku sama dengan mode dummy.
   * Return true bila ada data untuk dirender, false bila tidak.
   */
  async function pastikanAdaDataUntukFilter(paksa) {
    // [#3] paksa=true → SELALU ambil ulang data dari server, walau sudah ada
    // hasil di memori (dipakai tombol "Lacak 🔍" pencarian terpadu).
    if (!paksa && hasilTerakhir && hasilTerakhir.length > 0) return true;
    ...
    // Mode produksi: ambil daftar resi 30 hari terakhir dari WF1.
    judulHasil.classList.remove('hidden');
    judulHasil.textContent = 'Memuat data 30 hari terakhir...';
    renderSkeleton(2);
    try {
      const resp = await fetchWithRetry(CONFIG.N8N_GET_URL, { method: 'GET', headers: { 'Accept': 'application/json' } });
      ...
      const results = data && Array.isArray(data.results)
        ? data.results
        : (data && Array.isArray(data.data) ? data.data : []);
      hasilTerakhir = results.map((item) => normalisasiResult(item, item && item.no_resi ? item.no_resi : ''));
      sumberCache = false;
      waktuCache = null;
      sumberSemuaResi = true;
      if (hasilTerakhir.length === 0) {
        hasilContainer.innerHTML = '';
        judulHasil.classList.add('hidden');
        tampilToast('Belum ada resi yang diupdate dalam 30 hari terakhir.', 'info');
        return false;
      }
      return true;
    } catch (err) {
  ...
  /**
   * [GABUNG 2026-07-06] PENCARIAN TERPADU — satu-satunya pintu masuk pencarian.
   * Membaca keempat field sekaligus: no resi + dari tanggal + sampai tanggal
   * + status. Aturan:
   * - No resi terisi   → lacak resi tsb (data segar); filter tanggal/status
   *   tetap diterapkan oleh renderHasil lewat bacaFilterEfektif.
   * - No resi kosong   → muat daftar resi 30 hari terakhir dari server,
   *   lalu terapkan filter yang terisi.
   * - Semua kosong     → default: isi field tanggal dengan rentang 30 hari
   *   terakhir secara nyata, lalu tampilkan semuanya.
   */
  async function lakukanPencarian() {
    halamanSaatIni = 1; // pencarian/filter baru → mulai dari halaman 1
    const daftarResiInput = parseResi(inputResi.value);

    // [#3] SELALU inquiry ulang ke Google Sheet setiap klik "Lacak".
    if (daftarResiInput.length > 0) {
      await lakukanLacak();
      return;
    }
    ...
    // Tanpa resi spesifik → muat ulang daftar 30 hari terakhir dari server (paksa=true).
    if (!(await pastikanAdaDataUntukFilter(true))) return;
    ...
    if (kosong) {
      ...
      renderHasil(true);
      tampilToast('Menampilkan data 30 hari terakhir.', 'info');
    } else {
      renderHasil(false);
      tampilToast('Pencarian dengan filter diterapkan.', 'info');
    }
  }
  ...
  btnResetFilter.addEventListener('click', () => {
    ...
    halamanSaatIni = 1;
    renderHasil(false);
    tampilToast('Form pencarian dibersihkan.', 'info');
  });
