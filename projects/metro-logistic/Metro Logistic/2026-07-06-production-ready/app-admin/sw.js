/*
 * Service Worker — APP ADMIN LAPANGAN Metro Logistik
 * (Redesain & Mode Uji Dummy oleh Antigravity)
 */

'use strict';

const CACHE_VERSION = 'v2.0.0-admin';
const SHELL_CACHE = `metroadmin-shell-${CACHE_VERSION}`;
const API_CACHE = `metroadmin-api-${CACHE_VERSION}`;
const CDN_CACHE = `metroadmin-cdn-${CACHE_VERSION}`;
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        keys.map((key) => {
          const milikApp = key.startsWith('metroadmin-');
