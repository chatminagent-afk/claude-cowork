...
def peringatan_cover(data, no_wa=None):
    """Deck tanpa nama bisnis tetap jadi — tapi tidak boleh lolos tanpa disadari.

    Dicetak PALING AKHIR supaya ini yang terbaca di layar sebelum generate-deck.bat
    berhenti di 'pause'. Sengaja BUKAN error: cover generik masih layak kirim, yang
    tidak layak adalah Steven baru tahu setelah decknya sampai ke klien.
    """
...
def main():
    ...
    data = turunan(data)
    nama = re.sub(r"[^\w-]+", "-", data.get("nama_bisnis") or "tanpa-nama").strip("-").lower()
