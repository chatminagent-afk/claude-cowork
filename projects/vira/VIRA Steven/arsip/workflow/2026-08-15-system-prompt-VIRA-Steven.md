> **KEDALUWARSA 2026-08-30** — digantikan `2026-08-30-system-prompt-VIRA-Personal.md`.
> Perubahan: TINGKAT 2B (dua angka ROI) + TINGKAT 1 bukan lagi gerbang emisi tag.
> Lihat `docs/2026-08-30-perbaikan-brief-deck.md`.

# System Prompt — VIRA Steven

> File ini adalah **sumber tunggal** system prompt yang dipakai node AI Agent di n8n.
> Kalau kamu mengubahnya di sini, ubah juga di workflow (atau re-import).
> Bagian di dalam `{{ }}` diisi otomatis oleh workflow saat runtime — jangan diganti manual.
...
| Placeholder | Diisi oleh node | Isi |
|---|---|---|
| `prospect_context` | `Rakit Konteks` | Baris STATS prospek ini + `IS_NEW_USER` |
| `brief_context` | `Rakit Konteks` | Kolom STATS `brief_terisi` — daftar isian brief yang sudah tertangkap, supaya bot tidak menanyakan ulang |
| `about_context` | `Rakit Konteks` | Tab ABOUT_STEVEN, hanya baris `Boleh Dibagikan = Ya`, plus baris PANTANGAN sebagai aturan |
| `program_context` | `Rakit Konteks` | Tab PROGRAM, tanpa kolom `Catatan Internal` |
| `links_context` | `Rakit Konteks` | Tab LINKS `Status = Aktif`, hanya kolom Nama Link + Deskripsi |
| `faq_context` | `FAQ Retrieve` | Top-N baris FAQ paling relevan dengan pesan ini |

**Penting:** kolom `Catatan Internal` (PROGRAM) dan baris `Boleh Dibagikan = Tidak` (ABOUT_STEVEN)
tidak pernah dikirim ke AI sebagai fakta yang boleh dibagikan — baris PANTANGAN dikirim sebagai aturan,
bukan sebagai informasi. Ini mencegah bocornya catatan internal ke prospek.
