...
Yang dijamin di sini:
 ...
 10. Gerbang kelengkapan — nama_bisnis/industri/masalah_utama kosong = berhenti sebelum
     menulis apa pun, kecuali Steven mengizinkan per field (--lanjut-tanpa); kirim_deck ikut menolak
...
# masalah_utama sengaja dikosongkan -> sejak 2026-09-18 butuh izin eksplisit
rb = jalankan("_uji-gerbang.json", ["--lanjut-tanpa", "masalah_utama"])
...
r_nn = jalankan("_uji-gerbang.json", ["--lanjut-tanpa", "nama_bisnis"])   # izin Steven
...
cek("peringatan COVER GENERIK tercetak", "COVER GENERIK" in (r_nn.stdout or ""),
    (r_nn.stdout or "")[-300:])
cek("izin cover generik ikut tercatat di akhir keluaran",
    "DIIZINKAN KOSONG oleh Steven: nama_bisnis" in (r_nn.stdout or ""), (r_nn.stdout or "")[-300:])
...
json.dump(d, io.open(bolong, "w", encoding="utf-8"), ensure_ascii=False)
jalankan("_uji-gerbang.json", ["--lanjut-tanpa", "masalah_utama"])
...
# -- 7. Gerbang kelengkapan (2026-09-18) --
# Deck uji 17/09 terkirim sebagai tanpa-nama.pdf: nama_bisnis kosong hanya menghasilkan
# peringatan di baris terakhir. Sejak 2026-09-18 tiga field wajib menghentikan generator
# SEBELUM menulis apa pun, kecuali Steven mengizinkan per field (--lanjut-tanpa).
print("\n[7] Gerbang kelengkapan")
d7 = json.load(io.open(BRIEF, encoding="utf-8"))
d7["nama_bisnis"] = "belum disebut"
d7["industri"] = ""
d7["no_wa"] = "6289900001168"
json.dump(d7, io.open(bolong, "w", encoding="utf-8"), ensure_ascii=False)
jejak = [os.path.join(DIR, "keluaran", "tanpa-nama-1168" + x) for x in (".html", "-pratinjau.html", ".pdf")]
for f in jejak:
    if os.path.exists(f):
        os.remove(f)
r7 = jalankan("_uji-gerbang.json")
o7 = r7.stdout or ""
cek("nama_bisnis + industri kosong -> berhenti (exit 2)", r7.returncode == 2, o7[-300:])
cek("tidak ada berkas keluaran yang ditulis", not any(os.path.exists(f) for f in jejak))
cek("laporan menyebut dua field wajib + pertanyaan siap kirim",
    "BLOKIR     nama_bisnis" in o7 and "BLOKIR     industri" in o7
    and 'tanya: "Nama usahanya apa kak?' in o7, o7[:600])
cek("saran --lanjut-tanpa menyebut persis field yang kosong",
    "--lanjut-tanpa nama_bisnis,industri" in o7, o7[-300:])
r7b = jalankan("_uji-gerbang.json", ["--lanjut-tanpa", "nama_bisnis"])
cek("izin untuk nama_bisnis saja -> industri tetap menghentikan",
    r7b.returncode == 2 and "DIHENTIKAN: industri kosong" in (r7b.stdout or ""), (r7b.stdout or "")[-300:])
r7c = jalankan("_uji-gerbang.json", ["--lanjut-tanpa", "nama_bisnis,industri"])
cek("izin untuk keduanya -> deck jadi (exit 0)", r7c.returncode == 0, (r7c.stderr or r7c.stdout or "")[-300:])
cek("berkas tanpa nama bisnis diberi 4 digit akhir no_wa (tidak saling timpa)", os.path.exists(jejak[0]))
cek("izin tercatat di akhir keluaran",
    "DIIZINKAN KOSONG oleh Steven: industri, nama_bisnis" in (r7c.stdout or ""), (r7c.stdout or "")[-300:])
r7d = jalankan("_uji-gerbang.json", ["--lanjut-tanpa", "deskripsi_bisnis"])
cek("--lanjut-tanpa untuk field non-wajib ditolak",
    r7d.returncode not in (0, 2) and "hanya untuk field" in ((r7d.stderr or "") + (r7d.stdout or "")),
    (r7d.stderr or "")[-300:])
for f in jejak:
    if os.path.exists(f):
        os.remove(f)

d7p = json.load(io.open(BRIEF, encoding="utf-8"))
d7p["nama_bisnis"] = "Uji Gerbang"
d7p["deskripsi_bisnis"] = ""
d7p["volume_chat_harian"] = "puluhan"
json.dump(d7p, io.open(bolong, "w", encoding="utf-8"), ensure_ascii=False)
r7e = jalankan("_uji-gerbang.json")
o7e = r7e.stdout or ""
cek("field penentu lain kosong -> deck TETAP jadi", r7e.returncode == 0, (r7e.stderr or o7e)[-300:])
cek("... tapi dilaporkan sebagai PERINGATAN beserta dampaknya",
    "PERINGATAN deskripsi_bisnis" in o7e and "slide Konteks Bisnis dibuang" in o7e, o7e[:600])
cek("volume tanpa angka ('puluhan') dihitung kosong", "tidak ada angkanya: 'puluhan'" in o7e, o7e[:600])

NADIA = {"no_wa": "6285171701168", "nama": "Nadia", "nama_bisnis": "", "industri": "katering",
         "masalah_utama": "chat numpuk di malam hari", "aksi_utama": "order", "channel": "WhatsApp",
         "alur_setelah_chat": "tanya-tanya dulu soal menu dan harga, kalau sudah fix langsung order",
         "pertanyaan_tersering": "ketersediaan tanggal dan harga paket",
         "volume_chat_harian": "sekitar 20-30 chat", "sumber_prospek": "Organik",
         "deskripsi_bisnis": "belum disebut"}
k7 = bd.cek_kelengkapan(NADIA, bd.slug_berkas(NADIA))
per = lambda t: [x["field"] for x in k7 if x["tingkat"] == t]  # noqa: E731
cek("brief uji 17/09: yang menghentikan hanya nama_bisnis", per("BLOKIR") == ["nama_bisnis"], str(k7))
cek("brief uji 17/09: deskripsi_bisnis jadi PERINGATAN", per("PERINGATAN") == ["deskripsi_bisnis"], str(k7))
cek("pertanyaan_tersering satu butir dicatat sebagai mockup tipis",
    "pertanyaan_tersering" in per("CATATAN"), str(k7))
cek("slug: nama bisnis dipakai kalau ada", bd.slug_berkas({"nama_bisnis": "Dapur Nadia"}) == "dapur-nadia")
cek("slug: 'belum disebut' dianggap kosong -> 4 digit akhir no_wa",
    bd.slug_berkas({"nama_bisnis": "belum disebut", "no_wa": "6281234565678"}) == "tanpa-nama-5678")
cek("slug: tanpa nama & tanpa no_wa -> tanpa-nama", bd.slug_berkas({}) == "tanpa-nama")
_nk = os.path.join(DIR, "naskah", "uji-naskah-gerbang.json")
json.dump({"gelembung": [{"arah": "masuk", "teks": "Halo kak?"}]}, io.open(_nk, "w", encoding="utf-8"))
k7n = bd.cek_kelengkapan(dict(NADIA, pertanyaan_tersering=""), "uji-naskah-gerbang")
cek("naskah tulisan tangan menggantikan pertanyaan_tersering (tidak diperingatkan)",
    "pertanyaan_tersering" not in [x["field"] for x in k7n], str(k7n))
os.remove(_nk)

# Dampak yang dilaporkan harus sesuai template: slide yang disebut 'dibuang' memang
# ber-data-butuh field itu, dan field 'umum' memang punya kalimat cadangan.
_butuh = {sid: b for sid, b in _re.findall(r'data-id="([^"]+)"[^>]*data-butuh="([^"]*)"', tpl)}
cek("pain-points dibuang karena masalah_utama", _butuh.get("pain-points") == "masalah_utama", str(_butuh))
cek("konteks-bisnis-custom dibuang karena deskripsi_bisnis",
    _butuh.get("konteks-bisnis-custom") == "deskripsi_bisnis", str(_butuh))
cek("#2 zero leaking profit dibuang karena industri", _butuh.get("2-zero-leaking-profit") == "industri", str(_butuh))
cek("#1 & #3 dibuang karena volume",
    _butuh.get("1-time-freedom") == _butuh.get("3-scalability") == "volume_angka", str(_butuh))
_cadangan = set(f for f, dft in bd.RX_FIELD.findall(tpl) if dft)
for f in ("aksi_utama", "alur_setelah_chat", "target_pelanggan", "sumber_prospek",
          "channel_utama", "biaya_admin_frasa"):
    cek("field '%s' punya kalimat cadangan di template" % f, f in _cadangan)

# kirim_deck: dijalankan di dalam proses dengan sheet dipalsukan dan CONFIG TANPA
# kredensial Kirimi — jadi apa pun hasilnya, tidak ada yang bisa terkirim.
import contextlib  # noqa: E402
import kirim_deck as kd  # noqa: E402
_pdf_uji = os.path.join(DIR, "_uji-kirim.pdf")
io.open(_pdf_uji, "wb").write(b"%PDF-1.4 uji")
_brief_kirim = dict(NADIA, _baris=2, update_terakhir="", deck_dikirim_ts="")
kd.vs.cari_request = lambda wa: (list(_brief_kirim), dict(_brief_kirim))
kd.vs.config = lambda: {"admin_phone": "6280000000000"}


def kirim(*arg):
    buf = io.StringIO()
    sys.argv = ["kirim_deck.py", "--wa", "6285171701168", "--file", _pdf_uji] + list(arg)
    try:
        with contextlib.redirect_stdout(buf):
            kd.main()
        return "selesai", buf.getvalue()
    except SystemExit as e:
        return str(e.code), buf.getvalue()


_h, _o = kirim()
cek("kirim_deck pratinjau: menampilkan DIBLOKIR ke klien",
    _h == "selesai" and "DIBLOKIR ke klien: nama_bisnis" in _o, _o[-300:])
_h, _o = kirim("--kirim")
cek("kirim_deck ke klien tanpa izin -> ditolak sebelum kirim", "field wajib kosong tidak dikirim" in _h, _h)
_h, _o = kirim("--ke-admin", "--kirim")
cek("kirim_deck ke Steven (review) tidak diblokir gerbang", "kirimi_user_code kosong" in _h, _h)
_h, _o = kirim("--kirim", "--lanjut-tanpa", "nama_bisnis")
cek("kirim_deck ke klien dengan izin Steven -> lolos gerbang", "kirimi_user_code kosong" in _h, _h)
os.remove(_pdf_uji)
...
for f in (bolong, os.path.join(DIR, "keluaran", "uji-gerbang.html"),
          ...
          os.path.join(DIR, "keluaran", "akademi-arsi.html"),
          os.path.join(DIR, "keluaran", "akademi-arsi-pratinjau.html")):
    if os.path.exists(f):
        os.remove(f)
